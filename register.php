...
$msg="";
 ...
 if(isset($_REQUEST['reg'])){
  ...
  if(!empty($name) && !empty($email) && !empty($phone) && !empty($pass) && !empty($utype) && !empty($gender) && !empty($uimage)){
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
       ...
       $enc_password = sha1($pass);
       $verification_code = bin2hex(random_bytes(20));
        $sql = "INSERT into user(uname, uemail, uphone, upass, utype, gender, uimage, verification_code, verification_status) VALUES('{$name}', '{$email}', '{$phone}', '{$enc_password}', '{$utype}', '{$gender}', '{$uimage}', '{$verification_code}', '0')";

       $insert_query =  mysqli_query($con, $sql);
       move_uploaded_file($temp_name1,"admin/user/$uimage");
       
            include_once "php/mail_script.php";
